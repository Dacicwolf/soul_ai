...
import { Briefcase, Users, Sparkles, ChevronRight, ArrowLeft } from 'lucide-react';
      ...
      <div className="relative z-10 flex-1 flex flex-col max-w-md mx-auto w-full">
        ...
        >
          <div className="flex items-center mb-4">
            <button
              onClick={() => navigate('/conversation-selector')}
        ...
        <div className="flex-1 flex flex-col justify-center space-y-4">
          {modes.map((mode, index) => (
            <motion.button
              ...
              className={`w-full p-3 rounded-2xl text-left transition-all duration-200 border hover:brightness-105 ${selectedMode === mode.id ? 'shadow-lg' : 'shadow-sm'
                }`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className={`font-medium text-base mb-0.5 ${selectedMode === mode.id ? 'text-white' : 'text-[#222]'
                    }`}>
                    {mode.title}
                  </h3>
                  <p className={`text-xs mb-0.5 leading-snug ${selectedMode === mode.id ? 'text-white/90' : 'text-[#222]/80'
                    }`}>
                    {mode.description}
                  </p>
                  <p className={`text-xs leading-snug ${selectedMode === mode.id ? 'text-white/70' : 'text-[#222]/60'
                    }`}>
                    {mode.details}
                  </p>
                </div>
                <ChevronRight className={`w-4 h-4 transition-transform flex-shrink-0 mt-1 ${selectedMode === mode.id ? 'text-white translate-x-1' : 'text-[#222]/40'
                  }`} />
              </div>
            </motion.button>
          ))}
